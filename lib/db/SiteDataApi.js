...
function getSiteInfo({ collection, block, NOTION_CONFIG }) {
  ...
  const defaultPageCover = SITE_IMAGES.HOME_BANNER_IMAGE || NOTION_CONFIG?.HOME_BANNER_IMAGE || '/bg_image.jpg'
  const defaultIcon = SITE_IMAGES.AVATAR || NOTION_CONFIG?.AVATAR || '/avatar.svg'
  ...
  // Notion 封面和图标存储在 format 字段中
  const pageCover = collection?.format?.page_cover
    ? mapImgUrl(collection?.format?.page_cover, collection, 'collection')
    : defaultPageCover

  // 用户头像压缩一下
  let icon = compressImage(
    collection?.format?.page_icon
      ? mapImgUrl(collection?.format?.page_icon, collection, 'collection')
      : defaultIcon
  )
  // 站点网址
